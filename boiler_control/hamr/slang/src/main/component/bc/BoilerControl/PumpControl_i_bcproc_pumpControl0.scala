...
object PumpControl_i_bcproc_pumpControl0 {
  ...
  def timeTriggered(api: PumpControl_i_Operational_Api): Unit = {
    ...
    api.get_openPump() match {
      ...
      case Some(command) =>
        api.logInfo(s"Pump0: ${if (command) "Open" else "Close"}")
        // tell pump what to do
        isPumpOpen = command
        // assume pump did what it was supposed to do
          // note, in future, state variable can randomize this unit testing and failure
          // when considering system level requirements.
        isPumpFlow = command
    }
