...
class PumpControl_i_bcproc_pumpControl0_GumboX_Tests extends PumpControl_i_bcproc_pumpControl0_GumboX_TestHarness_ScalaTest_Generator {
  ...
  override def getInitialiseProfiles: MSZ[PumpControl_i_bcproc_pumpControl0_Profile] = {
    return MSZ(getDefaultInitialiseProfile)
  }

  // profiles that will be used to generate the incoming port values
  override def getProfiles_P: MSZ[PumpControl_i_bcproc_pumpControl0_Profile_P] = {
    return MSZ(getDefaultProfile_P)
  }

  // profiles that will be used to generate the incoming port values
  // and the pre-state values of the state variables
  override def getProfiles_PS: MSZ[PumpControl_i_bcproc_pumpControl0_Profile_PS] = {
