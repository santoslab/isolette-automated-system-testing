...
class WaterMeter_i_bcproc_waterMeter_GumboX_Tests extends WaterMeter_i_bcproc_waterMeter_GumboX_TestHarness_ScalaTest_Generator {
  ...
  override def getInitialiseProfiles: MSZ[WaterMeter_i_bcproc_waterMeter_Profile] = {
    return MSZ(getDefaultInitialiseProfile)
  }

  // profiles that will be used to generate the incoming port values
  override def getProfiles_P: MSZ[WaterMeter_i_bcproc_waterMeter_Profile_P] = {
    return MSZ(getDefaultProfile_P)
  }

  // profiles that will be used to generate the incoming port values
  // and the pre-state values of the state variables
  override def getProfiles_PS: MSZ[WaterMeter_i_bcproc_waterMeter_Profile_PS] = {
