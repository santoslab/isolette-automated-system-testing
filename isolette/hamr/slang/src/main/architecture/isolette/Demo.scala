...
import art.scheduling.static.CliCommandProvider
import isolette.runtimemonitor.{GumboXRuntimeMonitor, HamrVision}
...
object Demo extends App {
  ...
  def defaultScheduler(): Scheduler = {
    ...
    //return Schedulers.getStaticSchedulerH(MNone())
  ...
  def main(args: ISZ[String]): Z = {
    Cli(' ').parseRun(args, 0) match {
      case Some(o: Cli.RunOption) =>
        ...
        GumboXRuntimeMonitor.threadNickNames = StaticSchedulerCust.revThreadNickNames
