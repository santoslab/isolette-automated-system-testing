...
if (platform == "JVM") {
  ...
  println(s"Note: runtime monitoring has not been enabled as it's not yet supported for $platform")
}
...
if (results.ok) {
  val hamrBinDir = aadlDir.up / "hamr" / "slang" / "bin"
  proc"${hamrBinDir / "sergen-sys.cmd"}".echo.console.runCheck()
  proc"${hamrBinDir / "slangcheck-sys.cmd"}".echo.console.runCheck()
}
